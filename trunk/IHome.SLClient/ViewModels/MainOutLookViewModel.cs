...
namespace IHome.SLClient
{
    ...
    public class MainOutLookViewModel : INotifyPropertyChanged
    {
        ...
        {
            string menulist = "["
                            + " {header:'房源管理',item_list:[{name:'新增房源',type_name:'IHome.SLClient.HouseManagement.HoseInfoAddView',xap_name:'IHome.SLClient.HouseManagement.zip'},{name:'查询房源'},{name:'房源跟单'},{name:'房源详情'},{name:'房源修改'},{name:'房源推送'},{name:'房源委托'},{name:'房源带看'},{name:'钥匙收据'},{name:'房源备案'},{name:'房源图片'}]}"
                            ...
                            + ",{header:'基础设置',item_list:[{name:'小区管理',type_name:'IHome.SLClient.InfoManagement.ConmmunityAddView',xap_name:'IHome.SLClient.InfoManagement.zip'},{name:'楼盘字典'},{name:'数据字典'},{name:'区域板块'},{name:'数据字典'},{name:'推送配置'},{name:'业务参数'},{name:'个人管理'},{name:'个人信息管理'},{name:'业务报表'}]}"
